	...
	try {
		const result = Bun.spawnSync({
			cmd: [
				...
				"-NonInteractive",
				"-EncodedCommand",
				powershellEncodedCommand(parserScript),
			],
			env: {
				...process.env,
				CODEX_GET_CONTENT_COMMAND: Buffer.from(command, "utf8").toString(
					"base64",
				),
			},
			stdout: "pipe",
...
export function rewritePowerShellGetContent(
	command: string,
	platform: NodeJS.Platform = process.platform,
	findOffsets: OffsetFinder = findGetContentInsertionOffsets,
): string | undefined {
	if (platform !== "win32" || !/\bGet-Content\b/i.test(command)) return;
	const offsets = [...new Set(findOffsets(command))]
		.filter((offset) => offset >= 0 && offset <= command.length)
		.sort((left, right) => right - left);
	if (offsets.length === 0) return;

	let rewritten = command;
	for (const offset of offsets)
		rewritten = `${rewritten.slice(0, offset)} -Encoding utf8${rewritten.slice(offset)}`;
