...
function formatMessage(arguments_: unknown[]): string {
	return arguments_
		.map((value) =>
			typeof value === "string" ? value : inspect(value, { depth: 6 }),
		)
		.join(" ");
}

export function platformFromArgv(
	argv: readonly string[] = process.argv,
): LogPlatform | undefined {
 ...
 * original methods back. Without `--claude` or `--codex`, this is a no-op.
 */
export function hijackConsole(options: LoggerOptions = {}): ConsoleLogger {
	...
	) as Record<LogLevel, (...arguments_: unknown[]) => void>;
	let restored = false;
	...
	const soundOptions =
		typeof options.sound === "object" ? options.sound : undefined;
