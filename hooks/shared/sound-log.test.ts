...
test('maps Linux log levels to desktop sounds', () => {
  ...
  expect(commands).toEqual([['/usr/bin/canberra-gtk-play', '--id=dialog-error']]);
});
...
test('uses the macOS system sound for warnings', () => {
  ...
  expect(commands).toEqual([['/usr/bin/afplay', '/System/Library/Sounds/Funk.aiff']]);
});
...
test('uses PowerShell for Windows sounds', () => {
  ...
  emitLogSound('info', {
    platform: 'win32',
    resolveExecutable: (executable) => (executable === 'powershell.exe' ? 'C:/Windows/powershell.exe' : null),
    run: (command) => commands.push(command),
  });

  expect(commands).toEqual([['C:/Windows/powershell.exe', '-NoLogo', '-NoProfile', '-NonInteractive', '-Command', '[Console]::Beep(523, 120)']]);
});
