...
test('appends one valid JSON object per line', () => {
  ...
  try {
    ...
    expect(appendLogRecord(logFile, { ...record, message: 'second' })).toBe(true);
...
test('logging failures are fail-open', () => {
  const directory = mkdtempSync(join(tmpdir(), 'alux-log-failure-'));
  const parentFile = join(directory, 'not-a-directory');
  writeFileSync(parentFile, 'file');
  try {
    expect(
      appendLogRecord(join(parentFile, 'alux.jsonl'), {
        timestamp,
        platform: 'codex',
        level: 'error',
        message: 'ignored',
