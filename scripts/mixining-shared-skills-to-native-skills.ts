...
export async function mixSharedSkills(skillsRoot: string): Promise<MixSharedSkillsResult> {
  ...
  const targetDirectories = nativeDirectories;
...
function usage(): never {
  console.error('Usage: mixining-shared-skills-to-native-skills.ts [skills-root]');
  process.exit(2);
}
...
async function main(): Promise<void> {
  ...
  try {
    const skillsRoot = resolve(args[0] ?? join(dirname(import.meta.dir), 'skills'));
    const result = await mixSharedSkills(skillsRoot);
    console.log(`Copied ${result.copiedSkills.length} shared skill(s) to ${result.targetDirectories.length} native director(ies).`);
  } catch (error) {
