...
function usage(): never {
  console.error('Usage: svg-to-png.ts <input.svg> [output.png] [--force] [--render]');
  process.exit(2);
}
...
if (positional.length < 1 || positional.length > 2 || [...flags].some((flag) => flag !== '--force' && flag !== '--render')) {
  usage();
}

const inputPath = resolve(positional[0]);
const outputPath = resolve(positional[1] ?? `${inputPath.slice(0, -extname(inputPath).length)}.png`);
...
function extractWrapperPng(source: string): Uint8Array | undefined {
  const body = source
    ...
    .replace(/<!--[\s\S]*?-->/g, '')
    .trim();
  const wrapper = body.match(/^<svg\b[^>]*>\s*<image\b([^>]*)\/?>(?:\s*<\/image>)?\s*<\/svg>$/is);
  if (!wrapper) return undefined;

  const href = wrapper[1].match(/(?:href|xlink:href)\s*=\s*["']data:image\/png;base64,([^"']+)["']/i);
...
if (embedded) {
  ...
  const renderers: Array<[string, string[]]> = [
    ...
    ['inkscape', ['inkscape', inputPath, '--export-type=png', `--export-filename=${outputPath}`]],
    ['magick', ['magick', inputPath, outputPath]],
    ['convert', ['convert', inputPath, outputPath]],
  ];
  const renderer = renderers.find(([name]) => Bun.which(name));
  if (!renderer) {
    throw new Error('SVG needs rasterization, but no supported renderer was found. Install rsvg-convert, inkscape, or ImageMagick.');
  }
