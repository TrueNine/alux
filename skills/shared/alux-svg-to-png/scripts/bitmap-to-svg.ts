...
function usage(): never {
	console.error(
		"Usage: bitmap-to-svg.ts <input.png> [output.svg] [--force] [--packages-dir <dir>] [--colors <n>] [--pathomit <n>] [--ltres <n>]",
	);
	process.exit(2);
}
...
const outputPath = resolve(
	positional[1] ?? `${inputPath.slice(0, -extname(inputPath).length)}.svg`,
);
...
try {
	const packageBase = options["packages-dir"]
		? resolve(String(options["packages-dir"]))
		: undefined;
	const packageImport = (name: string) =>
		packageBase
			? import(
					pathToFileURL(Bun.resolveSync(name, `${packageBase}/runner.ts`)).href
				)
			: import(name);
	...
	throw new Error(
		"Vectorization packages are unavailable. Install them in a temporary directory with `bun add --cwd <dir> --no-save imagetracerjs pngjs`, then pass `--packages-dir <dir>`.",
		{ cause: error },
	);
}
const image = pngModule.PNG.sync.read(
	Buffer.from(await Bun.file(inputPath).arrayBuffer()),
);
const traceOptions = {
...
const svg = tracerModule.imagedataToSVG(
...
console.log(
	`${outputPath}: ${image.width}x${image.height}, traced with imagetracerjs`,
);
