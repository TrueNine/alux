...
<!-- END_SUBMODULE:frontend/AGENTS.md -->`;
  ...
  it('normalizes extended engineering vocabulary with longest matches', () => {
    ...
    expect(normalizeInstructionText('实体、服务、事件、技能、代理、输入、资源、补丁、校验与协议。')).toBe('eneity, service, event, skill, agent, input, source, patch, verify and protocol.');
  });
...
`,
  ...
  it('normalizes workflow, image, import, and adaptation vocabulary', () => {
    expect(normalizeInstructionText('工作流平台使用图片静态导入默认值。你是适配器,你适配。')).toBe('workflow platform 使用 image static import default value. you are adaptor, you adapt.');
  ...
  it('normalizes instruction structure and availability vocabulary', () => {
    ...
    expect(normalizeInstructionText('list impl integrate returns enabled ON disabled OFF def val obj')).toBe('list impl integrate returns enabled ON disabled OFF def val obj');
  });
...
`);
  ...
  it('preserves standalone words and asset paths while normalizing prose', () => {
    expect(normalizeInstructionText('specification ./assets/openai-logo.svg ./assets/jimmer-logo.png')).toBe('specification ./assets/openai-logo.svg ./assets/jimmer-logo.png');
  });
