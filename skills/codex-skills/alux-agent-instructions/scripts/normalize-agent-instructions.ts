...
const DEFAULT_DICTIONARY_PATH = resolve(
	SCRIPT_DIRECTORY,
	"../dictionary/vocabulary.yaml",
);
...
export function loadVocabulary(path = DEFAULT_DICTIONARY_PATH): Vocabulary {
	...
	for (const rule of [
		...parsed.literalRules,
		...forbiddenCharacterRules,
		...parsed.punctuationRules,
	]) {
	...
	return {
		...parsed,
		forbiddenCharacterRules: [...forbiddenCharacterRules].sort(
			(first, second) => second.from.length - first.from.length,
		),
		literalRules: [...parsed.literalRules].sort(
			(first, second) => second.from.length - first.from.length,
		),
	};
}
...
function replaceLiteralRule(content: string, rule: LiteralRule): string {
	...
	const leadingBoundary = /^[A-Za-z0-9]/.test(rule.from)
		? "(?<![A-Za-z0-9])"
		: "";
	const trailingBoundary = /[A-Za-z0-9]$/.test(rule.from)
		? "(?![A-Za-z0-9])"
		: "";
	const pattern = new RegExp(
		`${leadingBoundary}${escapeRegExp(rule.from)}${trailingBoundary}`,
		"g",
	);
...
function normalizeSpacing(content: string, vocabulary: Vocabulary): string {
	const tokens = [
		...
		...(vocabulary.forbiddenCharacterRules ?? [])
	...
	for (const token of tokens) {
		...
		normalized = normalized
			.replace(new RegExp(`(${HAN})(${escaped})`, "gu"), "$1 $2")
			.replace(new RegExp(`(${escaped})(${HAN})`, "gu"), "$1 $2");
	}
	const tokenPattern = tokens.map(escapeRegExp).join("|");
	const joinedTokens = new RegExp(
		`(?<![A-Za-z0-9])(${tokenPattern})(${tokenPattern})(?![A-Za-z0-9])`,
		"gu",
	);
	...
	do {
		...
		normalized = normalized.replace(
			joinedTokens,
			(match, firstToken: string, secondToken: string) =>
	...
	return normalized
		...
		.replace(/([,;:?])([A-Za-z\p{Script=Han}])/gu, "$1 $2")
		.replace(
			/\.(?!(?:md|toml|ya?ml|json|tsx?|jsx?|mjs|cjs|lock|io)\b)([A-Za-z\p{Script=Han}])/gu,
			". $1",
		);
}
...
function normalizeLine(line: string, vocabulary: Vocabulary): string {
	if (
		...
		/^<!-- (?:BEGIN_GLOBAL|BEGINE_GLOBAL|END_GLOBAL):~\/\.codex\/ -->$/.test(
			line,
		) ||
...
export function findNormalizationIssues(
	content: string,
	vocabulary: Vocabulary = DEFAULT_VOCABULARY,
): NormalizationIssue[] {
	const allowedSymbols = new Set([
		...vocabulary.allowedNonAsciiSymbols,
		...vocabulary.punctuationRules.map((rule) => rule.from),
	]);
...
export function normalizeInstructionText(
	...
	for (const rule of vocabulary.forbiddenCharacterRules ?? []) {
		globallyNormalized = globallyNormalized.replaceAll(
			rule.from,
			canonicalTranslation(rule),
		);
	}
...
export function normalizeGovernedFiles(
	projectRoot: string,
	mode: NormalizeMode,
	targetPath?: string,
): NormalizeResult {
	...
	const paths = targetPath
		? statSync(targetPath).isDirectory()
			? findGovernedFiles(projectRoot).filter((path) => {
					...
					return (
						pathFromTarget === "" ||
						(!pathFromTarget.startsWith("..") && !isAbsolute(pathFromTarget))
					);
				})
			: [targetPath]
	...
	for (const path of paths) {
		...
		if (after !== before) {
			...
			if (mode === "diff")
				diffs.push(unifiedDiff(projectRoot, path, before, after));
		}
...
export function runNormalizeCli(
